...
import ProgressBar from "@/components/appointments/ProgressBar";
      ...
      },
      {
        onSuccess: async (appointment) => {
          ...
          try {
            ...
            // try to parse JSON response; if empty, fallback to text
            ...
            if (!emailResponse.ok) {
              const statusInfo = `${emailResponse.status} ${emailResponse.statusText}`;
              const details =
                emailResult && Object.keys(emailResult).length
                  ? emailResult
                  : await emailResponse.text().catch(() => null);
              // stringify details for clearer console output
              console.error(
